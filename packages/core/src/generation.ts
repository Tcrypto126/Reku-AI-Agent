...
export async function generateMessageResponse({
    ...
    while (true) {
        try {
            ...
            if (runtime.character.credits < 1) {
                parsedContent = {
                    user: "Eliza",
                    text: "Failed: You're out of credits! Please recharge!",
                    action: "CONTINUE",
                };
            } else {
                const response = await generateText({
                ...
                //Reduce credits when using OpenAI API on Telegram or Discord
                const newCharacter = {
                    ...runtime.character,
                    credits: (runtime.character.credits ?? 0) - 1,
                    settings: {
                        ...runtime.character.settings,
                        secrets: {
                            TELEGRAM_BOT_TOKEN:
                                runtime.character.telegramBotToken ?? "",
                            DISCORD_API_TOKEN:
                                runtime.character.discordAPIToken ?? "",
                        },
                    },
                };
                ...
                if (user == "telegram") {
                    try {
                        const response = await fetch(
                            `${BASE_URL}:${SERVER_PORT}/agents/${runtime.agentId}/set`,
                            options
                        );
