...
import { Token } from "../../packages/plugin-tron/src/interfaces/symbiosis.ts";
...
export async function initializeClients(
    ...
    const clientTypes: string[] =
        character.clients?.map((c) => c.toLowerCase()) || [];
...
const handlePostCharacterLoaded = async (
    ...
    // console.log("processedCharacter: ", processedCharacter);
...
const startAgents = async () => {
    ...
    directClient.startAgent = async (character) => {
        ...
        // Handle Post Processors plugins
        if (character.postProcessors?.length > 0) {
            console.log(character.postProcessors);
            elizaLogger.info(
                character.name,
                "loading postProcessors",
                character.postProcessors
